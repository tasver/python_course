...
class Employee_monthly(Employee):
...
class Freelancer(Employee):
    def __init__(self, name: str, line_rate: int, number_lines: int) -> None:
        self.name = name
...
def employees_creating() -> list:
    names = ('Oleg', 'Petro', 'Ivan', 'Andriy', 'Vasia', 'Petya')
    employees = []
    for each in range(4):
        name = random.choice(names)
        employees.append(random.choice([
             Employee_hourly(name, random.randint(100, 200)),
             Employee_monthly(name, random.randint(10000, 25000)),
